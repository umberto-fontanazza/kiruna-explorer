...
app.get("/documents/:id", (req, res) => {
  ...
  const document = mockDB.documents.find(
...
app.post("/documents", (req, res) => {
  ...
  res
    .status(201)
    .json({ message: "Document created successfully", id: newDocument.id });
});
...
app.delete("/documents/:id", (req, res) => {
  ...
  const index = mockDB.documents.findIndex(
    (doc) => doc.id === parseInt(req.params.id),
  );
  if (index !== -1) {
